...
const CATEGORY_META = {
  ...
  volunteer: {
    ...
    accentBorder: "hover:border-emerald-300 border-l-4 border-l-emerald-500"
  },
  fundraising: {
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="text-2xs font-medium text-slate-400 flex items-center gap-0.5">
                  <Star className="w-3 h-3 text-emerald-500 fill-emerald-500" /> Impact: {recommendation.impact}
                </span>
                <span className="text-2xs font-medium text-slate-400 flex items-center gap-0.5">
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-all outline-none ${
              isSelected
                ...
                : disableSelection
                  ...
                  : 'bg-slate-50 hover:bg-emerald-50 text-slate-600 hover:text-emerald-700 border border-slate-200 hover:border-emerald-200'
            }`}
          >
            {isSelected ? (
              <>
                ...
                <ul className="space-y-1.5">
                  {recommendation.actionSteps.map((step, idx) => (
                    <li key={idx} className="text-xs text-slate-600 flex items-start gap-2 leading-relaxed">
                      <span className="font-display font-bold text-emerald-600 bg-emerald-50 rounded-full w-4 h-4 flex items-center justify-center text-3xs mt-0.5 flex-shrink-0">
                        {idx + 1}
                      </span>
                      {step}
                    </li>
            ...
            {onAddNote && isSelected && (
              <div id={`notes-container-${recommendation.id}`} className="mt-4 pt-3 border-t border-slate-100" onClick={(e) => e.stopPropagation()}>
                ...
                <textarea
                  ...
                  className="w-full text-xs p-2.5 bg-slate-50 border border-slate-200 rounded-lg focus:bg-white focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 outline-none resize-none transition-all text-slate-700"
                />
              </div>
            )}
          </div>
